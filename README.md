...
3. **선호도 선택** (`aesthetic.tsx`)
   - 가볍게 / 적당히 / 깊이 있게
   - AI 답변의 상세도 조절

4. **갤러리 선택** (`gallery.tsx`)
   - Supabase에서 갤러리 목록 조회
   - 선택한 갤러리 저장
...
- `app/main.py`: FastAPI 앱 진입점, CORS 설정, 라우터 등록
...
- `app/services/artworks/embedding_image.py`: 이미지 임베딩 생성
- `app/core/config.py`: 환경변수 관리 (Pydantic Settings)

### 모바일 앱

- `app/_layout.tsx`: 루트 레이아웃, 네비게이션 애니메이션 설정
- `app/(onboarding)/splash.tsx`: 스플래시 화면 (직사각형 회전 애니메이션)
